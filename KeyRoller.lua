...
local function UpdateKeyList(content)
    if text == nil then 
        -- initialization of text var
        -- the text var needs to be reused to ensure there is no stacking text in the content frame
    ...
    for player, key in pairs(playerKeys) do
        if key.level >= minKeyLevel and key.level <= maxKeyLevel then
            ...
            formattedDisplay = formattedDisplay .. string.format("%s: %s +%d", player, key.dungeon, key.level) .. '\n' .. '\n'
            --offset = offset + 20
            
        end
    end
    -- valuation of text var
    text:SetText(formattedDisplay)
    content:SetHeight(math.max(100, 1))
end
